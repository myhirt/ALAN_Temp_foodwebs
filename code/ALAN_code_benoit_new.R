library(ATNr)
#, lib.loc='~/share/groups/tib/ALAN_food_webs/')
library(ggplot2)
library(parallel)
set.seed(12)
...
run.light.gradient = function(param){
  ...
  model <- initialise_default_Unscaled_nuts(model, L, temperature = param$temp)
  model$S = rep(param$S, param$n_nut)
  model$q = rep(1.2, n_species - n_basal)
  
  # test if the separation between nocturnal and diurnal leads to new basal species
  overlaps = overlap.modification(x, param$period, 0)
  b2 = model$b
  b2[(n_basal +1):n_species, ] = b2[(n_basal +1):n_species, ] * overlaps
  if (any(colSums(b2) == 0.0)){return(NULL)}
  
  exts = sapply(light, run.light, model, 
                light.effect = param$light.effect, period = param$period)
  
  res = cbind.data.frame(t(exts), light, param$temp, param$light.effect, param$S, param$rep)
  names(res) = c("tot_ext", "pers_basals", "pers_night", "pers_cresp", "pers_day", 
...
run.light.gradient2 = function(param){
...
for (i in 1:reps){
  ...
  for (effect in effects){
    for (S in S.all){
      for (temp in temps) {
      n = n+1
      elem = list(light.effect = effect, rep = i, temp = temp, 
                  S = S, masses = masses, biomasses = biomasses,
                  n_species = n_species, n_basal = 20, n_nut = 2,
                  period = period, id = n)
      params[[n]] = elem
...
write.csv(yy, 'output/results_benoit_old.csv', row.names = F)
...
tab = read.csv("output/results_benoit_old.csv", header = T)
...
#results <- filter(tab, temperature == 15 & S ==1)

ggplot(filter(tab, temperature == "10"), aes(x=light, y = tot_ext, color = light.effect)) +
  #geom_jitter()+
...
ggsave(file = "output/results_new.pdf", height=6, width =8)
  
  ggplot(tab, aes(x=temperature, y = tot_ext, color = as.factor(light))) +
    #geom_jitter()+
    geom_smooth(method = "lm") +
    labs(x = "temperature") +
    facet_wrap(~light.effect) +
...
test <- lm(tot_ext ~ temperature* light,
           data = filter(tab, light.effect == "N.D"))
anova(test)
summary(test)


ggsave(file = "output/results_new_temp.pdf", height=6, width =8)
